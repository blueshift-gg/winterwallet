...
use winterwallet_client::{
    AccountEntry, AdvanceSender, DEFAULT_ADVANCE_COMPUTE_UNIT_LIMIT, PersistedAdvance, upsert,
...
fn build_message(
    ...
    for ix in instructions {
        let prog_idx = accounts
            ...
            .expect("program ID missing from account list; upsert bug")
            as u8;
        msg.push(prog_idx);
        ...
        for meta in &ix.accounts {
            let idx = accounts
                ...
                .expect("account missing from account list; upsert bug")
                as u8;
            msg.push(idx);
        }
